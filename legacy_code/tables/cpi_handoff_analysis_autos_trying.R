...
cpi <- cpi_data %>%
  ...
  mutate(Pchange12 = (lag(value,3)/lag(value, 15)-1)) %>%
...
item_basket <- c("Commodities less food and energy commodities","Shelter")
...
cpi_with_minus <- cpi %>% filter(item_name %in% item_basket) %>% select(date, item_name, Wchange1a, Wthree_monthsA, Wchange12) %>%
  rbind(minus_shelter) %>% arrange(date)

# Start selecting the observations to look for
most_recent <- cpi_with_minus %>% filter(date == max(date)) %>%
