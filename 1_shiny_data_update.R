...
BEA_format_date <- function(x) {
  ...
  if (identifier == "Q") {
    ...
    month <- ifelse(
      quarter == "1",
      "03",
      ifelse(quarter == "2", "06", ifelse(quarter == "3", "09", "12"))
    )
  }
...
load_flat_files <- function(
  location = "https://apps.bea.gov/national/Release/TXT/",
  type = "Q"
) {
  print(Sys.time())
  series <- read_csv(
    paste(location, "SeriesRegister.txt", sep = ""),
    show_col_types = FALSE
  ) %>%
    clean_names() %>%
    rename(series_code = percent_series_code)
  tables <- read_csv(
    paste(location, "TablesRegister.txt", sep = ""),
    show_col_types = FALSE
  ) %>%
    clean_names()
  print(paste("Loading ", type, " data"))
  data <- read_csv(
    paste(location, "nipadata", type, ".txt", sep = ""),
    show_col_types = FALSE
  ) %>%
    clean_names() %>%
    rename(series_code = percent_series_code)
  ...
  final_data <- data %>%
    left_join(series, by = "series_code") %>%
    separate_rows(table_id_line_no, sep = "\\|") %>%
    separate(
      table_id_line_no,
      into = c("table_id", "line_no"),
      sep = ":",
      remove = FALSE
    ) %>%
    left_join(tables, by = "table_id")
  print(Sys.time())
  return(final_data)
}
...
pce_density_values <- NIPA_monthly_data %>%
  ...
  mutate(
    Pchange1a = (value / lag(value, 1))^12 - 1,
...
write_csv(
  rbind(cpi_density_values, pce_density_values),
  file = "data/shiny_density_test.csv"
)
write_csv(
  rbind(
    cpi_density_values %>% mutate(Pvalues = round(Pvalues, digits = 4)),
    pce_density_values %>% mutate(Pvalues = round(Pvalues, digits = 4))
  ),
  file = "data/shiny_density_test.csv"
)
