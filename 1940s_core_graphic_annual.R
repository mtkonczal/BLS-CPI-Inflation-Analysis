...
graphic_title <- "2023's Disinflation is Like Supply-Driven World War II Reopening, Not the Demand Inertial 1970s"
...
food_series_graphic <- food_series %>%
  ...
  mutate(max_value = year == max(year),
...
food_series_graphic %>%
  ...
  labs(title = graphic_title,
       subtitle = subtitle,
       caption = "BEA, BLS, seasonally adjusted. CPI uses period M13, or annual average, for annual values. Author's Calculations. Mike Konczal, Roosevelt Institute.",
       x = "Unemployment Rate",
...
ggsave("graphics/core_1940s_single.png", dpi="retina", width = 12, height=6.75, units = "in")

ggsave("graphics/core_1940s_single_blog.png", dpi="retina", units = "px", width = 1200, height=675)